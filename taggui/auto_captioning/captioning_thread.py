    ...
    def run_captioning(self):
        ...
        for i, image_index in enumerate(self.selected_image_indices):
            ...
            try:
                ...
                print(
                    f"Skipping {image.path.name} because its file format is "
            ...
            tags = add_caption_to_tags(image.tags, caption, caption_position, model_id,)
            # tags are populated here.

            logger.info(f"tags: {', '.join(tags.tags)} model: {tags.model}")
            ...
            print(
                f"{image.path.name} ({perf_counter() - start_time:.1f} s):\n"
                f"{console_output_caption}"
            )
        if are_multiple_images_selected:
            captioning_end_datetime = datetime.now()
            total_captioning_duration = (
                captioning_end_datetime - captioning_start_datetime
            ).total_seconds()
            average_captioning_duration = (
                total_captioning_duration / selected_image_count
            )
            print(
                f"Finished captioning {selected_image_count} images in "
